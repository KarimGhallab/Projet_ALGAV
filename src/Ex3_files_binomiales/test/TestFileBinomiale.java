package Ex3_files_binomiales.test;
...
public class TestFileBinomiale {
	...
	public void initialize() {
		...
		int N = 100000;
		for (int i=N; i>=1; i--) {
			fileBinomiale = fileBinomiale.add(new TournoiBinomial(new CleInteger(i)));
			fileBinomiale = fileBinomiale.add(new TournoiBinomial(new CleInteger(i)));
		}
	...
	public void consIter() {
		...
		FileBinomiale.constItr(liste, (FileBinomiale) fileBinomialeConstIter);
		
		testStructureFile(fileBinomialeConstIter);
		
		for (int i=5000000; i>4000000; i--)
			fileBinomialeConstIter = fileBinomialeConstIter.add(new TournoiBinomial(new CleInteger(i)));
	...
	public void testUnion() {
		...
		for (int i=100000; i>0; i--)
			fileBinomialeUnion1 = fileBinomialeUnion1.add(new TournoiBinomial(new CleInteger(i)));
		
		for (int i=0; i<250000; i++)
			fileBinomialeUnion2 = fileBinomialeUnion2.add(new TournoiBinomial(new CleInteger(i)));
	...
	public void testAvecCle128Bit() {
		...
		for(ICle c : v)
			fileBinomiale128 = fileBinomiale128.add(new TournoiBinomial(c));
	...
	public void testClesAlea() {
		...
		for(int i=1; i<=nb; i++) {
			for(int j=0; j<tailles.length; j++) {
				...
				FileBinomiale.constItr(liste, (FileBinomiale) fileBinomialeAlea);
	...
	public void testStructureTournoi(ITournoiBinomial tournoi) {
		...
		for (int i=0; i<tournoi.getNbFils(); i++) {
			...
			// System.out.println("(" + racine + " <= " + filsActuel.getCle() + ")   ==> " + assertion);
